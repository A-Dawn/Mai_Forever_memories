...
- **灵活导入控制**：支持"只总结不导入"模式，可选择性地控制摘要是否导入知识库。
- **确认导入机制**：支持在导入知识库前向管理员发送确认请求，包含简短记忆总结，由管理员决定是否导入。
- **性能监控与安全防护**：
...
- **确认导入机制**：当启用 `confirm_import` 时，系统会在导入摘要到知识库前向管理员发送确认消息。消息包含：
  - 摘要类型（每日/每周/永久）
  - 简短的记忆总结（摘要内容的前200字符）
  - 确认命令说明
  - 超时提醒

  管理员可以使用 `/memories confirm <task_id> yes` 同意导入，或 `/memories confirm <task_id> no` 拒绝导入。如果在指定时间内（默认5分钟）未响应，系统将自动拒绝导入并清理相关文件。
...
- `auto_import`: 是否自动导入摘要到知识库（默认为 `true`）。
- `confirm_import`: 是否启用导入确认机制（默认为 `false`），启用后会在导入前向管理员发送确认请求。
- `confirm_stream`: 发送确认消息的目标流ID（默认为空，使用 `performance.admin_id`）。
- `confirm_timeout`: 确认超时时间（秒），超时后自动拒绝导入（默认为 `300`）。
...
- `/memories run_daily`: 立即为当前聊天运行每日摘要任务（会导入知识库）。
- `/memories run_weekly`: 立即为当前聊天运行每周摘要任务（会导入知识库）。
- `/memories now`: 立即为当前聊天生成永久的记忆（会导入知识库）。
- `/memories summarize_daily`: 立即为当前聊天生成每日摘要（仅总结，不导入知识库）。
- `/memories summarize_weekly`: 立即为当前聊天生成每周摘要（仅总结，不导入知识库）。
- `/memories summarize_now`: 立即为当前聊天生成永久记忆（仅总结，不导入知识库）。
- `/memories confirm <task_id> <yes/no>`: 确认或拒绝待导入的任务（仅管理员可用）。
...
5. **确认导入机制**：启用 `confirm_import` 后，摘要生成后不会立即导入知识库，而是等待管理员确认。这可以防止意外的知识库污染，但会增加响应延迟。请根据实际需求权衡使用。
